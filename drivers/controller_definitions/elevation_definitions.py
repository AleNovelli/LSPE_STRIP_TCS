...
#in_stop_button=2 #allows the user to stop the main sequence
...
in_user_azimuth_idle=10 #input coming from the client which signals that the elevation motor is idle
io_user_modbus_table=11 #signals when the user moved the modbus communication on table memory

io_user_12=12
io_user13=13
out_relay_0=14 #switching OFF these relays will turn off digital outputs (only referred in MONITOR)
out_relay_1=15 #switching OFF these relays will turn off digital outputs (only referred in MONITOR)
...
fault_acknowledge=22 #decribes if an error has been acknowledged
...
mc_warning=36 #This flag becomes 1 if the signal from the MC is lost
